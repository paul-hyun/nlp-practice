...
class Trainer:
    ...
    def train(self, train_loader, valid_loader, model_name):
        ...
        for epoch_idx in range(self.config.n_epochs):
            ...
            valid_loss = self._validate(valid_loader) if valid_loader else train_loss

            print(
                f"Epoch {epoch_idx + 1} | Train Loss: {train_loss:.3f} | Valid Loss: {valid_loss:.3f}"
            )
        ...
        self.model.load_state_dict(
            torch.load(os.path.join(self.config.output_dir, f"{model_name}.pt"))[
                "model"
            ]
        )
